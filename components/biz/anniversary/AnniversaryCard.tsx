import { Clock, Heart, MapPin, Share2, Star, Bell } from "lucide-react";
...
import { EventType, RepeatType, ResourceType } from "@/lib/constant";
...
export function AnniversaryCard({
  ...
  const { calcDiffDays } = useAnnivCalc(anniversary);
  const { formatAnnivTriggerTime } = useAnniv();
  const [diffDays, pastDays] = calcDiffDays();
  const showNextTrigger =
    anniversary.repeat_type !== RepeatType.NONE &&
    anniversary.type !== EventType.BIRTHDAY;
    ...
    >
      ...
      <div
        className={cn(
          ...
          anniversary.type === 1 && "bg-anniversary",
          anniversary.type === 2 && "bg-birthday",
          anniversary.type === 3 && "bg-countdown"
        )}
      />
      ...
      <div className="flex items-stretch pl-6">
        ...
        <div className="flex flex-col items-center justify-center pr-6 border-r border-border/50 w-[80px]">
          ...
          {showNextTrigger && (
            <div className="mt-3 pt-3 border-t border-border/50 w-full text-center">
              <div className="flex items-center justify-center gap-1 text-primary">
                <span className="text-lg font-bold ">{pastDays}</span>
                <span className="text-xs">天</span>
              </div>
              <span className="text-[10px] text-muted-foreground">已过去</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 pl-6 min-w-0">
          ...
          <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
            ...
            {/* {anniversary.tags.length > 0 && (
              <span className="text-primary">#{anniversary.tags[0]}</span>
            )} */}
          </div>
          ...
          <div className="mt-3 flex items-center gap-1 -ml-2">
            ...
            >
              ...
              />
              <span className="text-xs tabular-nums">{anniversary.stats.like_cnt}</span>
            </Button>
            {/* <Button
              variant="ghost"
              size="sm"
