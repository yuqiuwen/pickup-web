import { BookHeart, Cake, AlarmClock, Shield, User, Crown } from "lucide-react";
...
export const EventTypeOptions = [
  {
    ...
    str_value: String(EventType.ANNIVERSARY),
    icon: BookHeart,
    color: "anniversary",
  },
  {
    ...
    str_value: String(EventType.BIRTHDAY),
    icon: Cake,
    color: "birthday",
  },
  {
    ...
    str_value: String(EventType.COUNTDOWN),
    icon: AlarmClock,
    color: "countdown",
  },
];
...
export const eventTypeMap = defineMap(EventTypeOptions, "value", [
  "label",
  "value",
  "color",
]);
...
export const RepeatTypeOptions = [
  {
    ...
    str_value: String(RepeatType.NONE),
  },
  {
    label: "每年",
    value: RepeatType.YEARLY,
    str_value: String(RepeatType.YEARLY),
  },
  {
    label: "每月",
    value: RepeatType.MONTHLY,
    str_value: String(RepeatType.MONTHLY),
  },
  {
    label: "每周",
    value: RepeatType.WEEKLY,
    str_value: String(RepeatType.WEEKLY),
  },
...
export const repeatTypeMap = defineMap(RepeatTypeOptions, "value", [
  "label",
  "value",
]);
...
export const StateTextMap: Record<number, string> = {
  [InviteState.PENDING]: "待发送",
  [InviteState.SENT]: "已发送",
  [InviteState.ACCEPTED]: "已接受",
  [InviteState.DECLINED]: "已拒绝",
  [InviteState.EXPIRED]: "已过期",
  [InviteState.CANCELLED]: "已撤销",
};
...
export const roleConfig = {
  [MemberRole.OWNER]: { label: "群主", icon: Crown, color: "text-yellow-500" },
  [MemberRole.MEMBER]: { label: "成员", icon: User, color: "text-muted-foreground" },
};
